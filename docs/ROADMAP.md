...
8. **Cross-project auto-delivery (tab-to-tab push)** — ✅ **shipped in v0.14.0.** `MainWindow.InboxDelivery.cs` sweeps every two seconds; the gate itself is `Firepit.Core.Inbox.InboxDeliveryPolicy`, kept free of UI and filesystem so the two invariants (never mid-turn, never twice) are unit-tested. Idle is `Embers` held across consecutive sweeps, which is trustworthy because the activity detector pins `Burning` while the agent reports progress over OSC 9;4. Off via `platform.inboxAutoDeliverEnabled`. Original design (worked out 2026-07-23, from the Umbrella-Bot/Bumblebee use case), implemented as written:
